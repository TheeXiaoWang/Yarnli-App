...
import { useLayerlineStore } from '../../stores/layerlineStore.js'
import { approximateTotalVolume } from '../../layerlines/intersections.js'
  ...
  return (
    <group>
      ...
      {layers.map((l, i) => {
        ...
        if (!ref) return null
        const label = `${src.fn} @ ${src.file} • ${src.kind}${src.localIndex!=null?` #${src.localIndex}`:''}`
        return (
      ...
      {/* DEBUG: show intersection boundary markers and cut loops if provided */}
      {(markers?.boundaries || []).map((b, bi) => (
        <group key={`bd-${bi}`}>
          {(b.points || []).map((p, pi) => (
