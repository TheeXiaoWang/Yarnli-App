...
/** Open-arc resampling: same step size L/N, but offset by half-step so
 * nodes are centered within the arc. Only intended for open polylines. */
export function resamplePolylineByArcLengthCentered(poly, count) {
  if (!Array.isArray(poly) || poly.length === 0 || count <= 0) return []
  const pts = poly.slice()
  // cumulative lengths (open – no wrap)
  const cum = [0]
  for (let i = 1; i < pts.length; i++) {
    const a = pts[i-1], b = pts[i]
    const d = Math.hypot(b[0]-a[0], b[1]-a[1], b[2]-a[2])
  ...
  const out = []
  for (let k = 0; k < count; k++) {
    const target = Math.max(0, Math.min(total, offset + k * step))
    // find segment containing target
    let i = 1
    while (i < cum.length && cum[i] < target) i++
    const i0 = Math.max(0, i - 1)
